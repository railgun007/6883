...
int Download_data(stock* newstock, string name, string startTime, string endTime, struct MemoryStruct& data, const char outfilename[FILENAME_MAX], CURL* handle, FILE* fp, CURLcode& result, string& sCookies, string& sCrumb)
{
	...
	int loop_limit = 100;
	while (execute_flag && loop_limit != 0)
	{
		try
		{
			free(data.memory);
...
int StockData::Download_stock(vector<pair<string, string>>& stock_list, map<string, stock*>& stock_map)
{
	...
	if (handle)
	{
		...
		while (itr != stock_list.end())
		{
			...
			if (Download_data(newstock, (*itr).first, startTime, endTime, data, outfilename, handle, fp, result, sCookies, sCrumb) != 1)
			{
...
void StockData::Multi_thread_Download_stock(vector<pair<string, string>>& stock_list)
{
	...
	{
		Sleep(1000);//make sure threads don't collide with each other (especially when dealing with files)
